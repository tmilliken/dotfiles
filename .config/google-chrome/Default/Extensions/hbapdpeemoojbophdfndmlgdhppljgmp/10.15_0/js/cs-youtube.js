var Tool = (function(){
  ...
  var renderVideoInsights = function(res){
    ...
    let html = [
      ...
      `<h3><img src="${chrome.extension.getURL('img/icon64.png')}"> Video Insights</h3>`,
      `<table>`,
        ...
        if (mutation.type === 'childList') {
          ...
          for (var i = 0, len = mutation.addedNodes.length; i < len; i++) {
            ...
            if (node.tagName === 'YTD-VIDEO-RENDERER' && node.getAttribute('hidden') === null) {
              ...
              processVideosTimer = setTimeout(runVideosProcessing, 500);
            }
          }
        }
      });
  ...
  var getAvgHtml = async function(data) {
    ...
    if (advancedMetricsLoading) {
      advancedBtnHtml = '<img class="xt-yt-avg-advanced-btn" src="' + chrome.extension.getURL('img/spinner32.gif') + '"><span class="xt-yt-avg-advanced-status"></span>';
    }
    var html = [
      '<div class="xt-close">✖</div>',
      advancedBtnHtml,
      `<h3><img src="${chrome.extension.getURL('img/icon64.png')}"> Search Insights</h3>`,
      '<table>',
      ...
      '<div class="xt-copy-export-bottom-row-left">',
        ...
        '<button class="xt-export-csv xt-ke-btn">' + Common.getIcon('export') + ' Export</button>',
      '</div>',
      '<div style="text-align:right"><a class="xt-yt-avg-full-btn" href="' + chrome.extension.getURL('html/ytstats.html?darkmode=' + isDarkMode()) + '" target="_blank">Detailed Breakdown</a></div>'
    ].join('\n');
    return html;
  };
  ...
  var showVideosSpinner = function(){
    $('#xt-yt-avg-widget').html('<img src="' + chrome.extension.getURL('img/spinner32.gif') + '">');
  };
  ...
  var getAdvancedMetricsClickHandler = function($root){
    ...
    if (videosObserver) videosObserver.disconnect();
    if (!$root) $root = $('body');
    $root.find('.xt-yt-avg-advanced-btn').replaceWith('<img class="xt-yt-avg-advanced-btn" src="' + chrome.extension.getURL('img/spinner32.gif') + '"><span class="xt-yt-avg-advanced-status"></span>');
    getAdvancedMetrics();
  };
  ...
  var processYtInitial = function(res){
    ...
    try {
      ...
      let likes = getObjValue(videoPrimaryInfoRenderer, '.videoActions.menuRenderer.topLevelButtons[0].toggleButtonRenderer.defaultText.accessibility.accessibilityData.label');
