var Common = (function(){
  ...
  var appendLTKBtn = function(html, params){
    ...
    let btnURL = chrome.extension.getURL(`html/page.html?page=autocomplete&query=${encodeURIComponent(params.query)}&service=${params.service}`);
  ...
  var appendListBtn = function(html, params){
    let country = (Starter.getSettings().country || '').toUpperCase();
    let countryTitle = country ? `(${country})` : '';
    let btnURL = chrome.extension.getURL(`html/page.html?page=list&query=${encodeURIComponent(params.query)}&service=${params.service}`);
    if (params.tld) btnURL += '&tld=' +  params.tld;
    if (params.lng) btnURL += '&lng=' +  params.lng;
    let title = '';
    if (params.title) title = params.title;
  ...
  var renderWidgetTable = function(rows, params){
    ...
    var html = [
      ...
      '</div>',
      '<h3 class="' + (json ? '' : 'xt-ke-h-no-credits') + '"><img src="' + chrome.extension.getURL('/img/icon24.png') + '" width="24" height="24" style="vertical-align:middle"> ' + title + '</h3>',
    ...
    html += '<div class="xt-widget-pagination">' + getPaginationHTML(keywordsPerPage, rows, 1) + '</div>';
    }
    ...
    $root.on('click', '[data-page]', function(e){
      ...
      var html = getPaginationHTML(perPage, rows, page);
    ...
    $root.on('change', '.xt-widget-page-select', function(e){
      ...
      var html = getPaginationHTML(perPage, rows, 1);
      $root.find('.xt-widget-pagination').html(html);
      var $table = $root.find('table');
  ...
  var getPaginationHTML = function(keywordsPerPage, rows, currentPage){
    var len = rows.length;
    var start = 1;
    var end = len;
    if (keywordsPerPage !== 'All') {
  ...
  const renderFindLongTailKeywordsBtn = function(params){
    ...
    $root.html(html);
    let btnURL = chrome.extension.getURL(`html/page.html?page=autocomplete&query=${encodeURIComponent(query)}&service=${params.service}`);
    if (params.tld) btnURL += '&tld=' +  params.tld;
    if (params.lng) btnURL += '&lng=' +  params.lng;
